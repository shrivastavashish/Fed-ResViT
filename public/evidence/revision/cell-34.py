...
def compute_metrics(y_true, y_pred, prefix=''):
    ...
    precision, recall, f1, _ = precision_recall_fscore_support(y_true, y_pred, labels=list(range(CONFIG['NUM_CLASSES'])), average='macro', zero_division=0)
    ...
    return {f'{prefix}accuracy': float(acc), f'{prefix}macro_precision': float(precision), f'{prefix}macro_recall': float(recall), f'{prefix}macro_f1': float(f1), f'{prefix}macro_specificity': float(specificity), f'{prefix}asr': float(asr) if not np.isnan(asr) else np.nan, f'{prefix}malignant_safety': float(malignant_safety) if not np.isnan(malignant_safety) else np.nan}
