...
def aggregate_geometric_median(client_deltas, max_iters=10, eps=1e-06):
...
class TrustAwareAggregator:
    ...
    def __init__(self, num_clients, t_low, t_high, momentum=0.85, flag_phi_threshold=0.5, flag_rounds=5, geom_median_iters=10, distance_mode='rms', adaptive_thresholds=True, mad_low_mult=0.5, mad_high_mult=3.0):
    ...
    def _round_thresholds(self, distances):
        ...
        if t_high <= t_low:
            t_high = t_low + max(abs(t_low) * 0.1, 1e-08)
        return (float(t_low), float(t_high))

    def aggregate(self, client_deltas, client_sizes):
        w_ref = aggregate_geometric_median(client_deltas, max_iters=self.geom_median_iters)
        ...
        self.reputation = self.momentum * self.reputation + (1.0 - self.momentum) * phi
        self.rounds_seen += 1
        for i in range(len(client_deltas)):
        ...
        info = {'distances': distances, 'phi': phi, 'reputation': self.reputation.copy(), 'weights': weights.copy(), 'flagged_now': flagged_now.copy(), 'distance_mode': self.distance_mode, 'round_t_low': round_t_low, 'round_t_high': round_t_high, 'distance_vector_l2_norm': float(np.linalg.norm(distances)), 'normalized_weights': weights / max(float(weights.sum()), 1e-12)}
