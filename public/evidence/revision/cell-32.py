class FedResViT(nn.Module):
    ...
    def __init__(self, num_classes=7, vit_backbone='vit_base_patch16_224', fusion_dim=512, dropout=0.3, resnet_pretrained=True, vit_pretrained=True, freeze_backbones=False):
        super().__init__()
        try:
        ...
        try:
            self.vit = timm.create_model(vit_backbone, pretrained=vit_pretrained, num_classes=0)
        except Exception as e:
            print('ViT pretrained load failed. Falling back to random init:', repr(e))
            self.vit = timm.create_model(vit_backbone, pretrained=False, num_classes=0)
        ...
        if freeze_backbones:
            ...
            if CONFIG.get('UNFREEZE_VIT_NORM', False) and hasattr(self.vit, 'norm'):
                for p in self.vit.norm.parameters():
                    p.requires_grad = True
        self.fusion = nn.Sequential(nn.Linear(cnn_in + vit_dim, fusion_dim), nn.ReLU(inplace=True), nn.Dropout(dropout), nn.Linear(fusion_dim, num_classes))
...
def build_model():
    model = FedResViT(num_classes=CONFIG['NUM_CLASSES'], vit_backbone=CONFIG['VIT_BACKBONE'], fusion_dim=CONFIG['FUSION_DIM'], dropout=CONFIG['DROPOUT'], resnet_pretrained=CONFIG['RESNET_PRETRAINED'], vit_pretrained=CONFIG['VIT_PRETRAINED'], freeze_backbones=CONFIG['FREEZE_BACKBONES'])
